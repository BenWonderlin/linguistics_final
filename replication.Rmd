...
hist(extension_data$systematicity)
```

```{r Extension 2}
extension_lm1 = extension_data %>%
  lm(systematicity ~ lexicalCategory + X30mos + phonemes + concreteness + logFreq + babyAVG,
     data = .)

summary(extension_lm1)
```
...
```{r}
collin.fnc(extension_data %>% dplyr::select(X30mos, phonemes, concreteness, logFreq, babyAVG))$cnumber
```
...
```{r}
extension_lm2 = extension_data %>%
  lm(systematicity ~ lexicalCategory + phonemes + logFreq + babyAVG,
     data = .)

summary(extension_lm2)
```
...
extension_lm3 = extension_data %>%
  lm(systematicity ~ lexicalCategory + X30mos + logFreq,
     data = .)

summary(extension_lm3)
...
AIC(extension_lm2, extension_lm3)
AICc(extension_lm2, extension_lm3)
BIC(extension_lm2, extension_lm3)
...
```{r}
extension_data$residSystematicity = resid(
  lm(data = extension_data, systematicity ~ lexicalCategory + logFreq + babyAVG)
)

extension_data %>%
  ggplot(., aes(x = phonemes, y = residSystematicity)) + geom_point() +
